...
public class Feature {
	...
	public int getLeftOrRight(int[][] grayImage, int[][] squares, int i, int j, float scale) {
		...
		//System.out.println("i+w : "+ (i+w) + " j+w : " + (j+w) + " j+h : " + (j+h));
		//System.out.println("i Lenfth: " + grayImage[0].length + " j length : " + grayImage[1].length);
		if(grayImage[0].length > i+w && grayImage[1].length > j+h)
		{
